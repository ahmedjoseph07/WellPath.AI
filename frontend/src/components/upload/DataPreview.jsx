  ...
  return (
    <div className="bg-geo-panel border border-geo-border rounded-xl p-4">
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-sm font-semibold text-geo-ink">Data Preview</h3>
        <span className="text-xs text-geo-muted bg-geo-soft px-2 py-0.5 rounded-full border border-geo-border">
      ...
      <div className="overflow-x-auto rounded-lg border border-geo-border">
        <table className="w-full text-xs">
          <thead>
            <tr className="bg-geo-soft border-b border-geo-border">
              {LABELS.map((label, i) => (
                <th key={i} className="px-3 py-2 text-left text-geo-muted font-semibold whitespace-nowrap">
                  {label}
                </th>
              ))}
            </tr>
          ...
          <tbody>
            {Array.from({ length: previewRows }).map((_, rowIdx) => (
              <tr
                key={rowIdx}
                className={`border-b border-geo-border/60 ${rowIdx % 2 === 0 ? 'bg-geo-panel' : 'bg-geo-soft/50'} hover:bg-geo-accent-soft transition-colors`}
              >
                {COLUMNS.map((col, colIdx) => {
                  const val = wellLog[col]?.[rowIdx]
                  return (
                    <td key={colIdx} className="px-3 py-1.5 text-geo-ink font-mono whitespace-nowrap">
